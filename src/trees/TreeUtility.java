...
public class TreeUtility {
	 ...
	 * 1,3,2,5,4
	 */
	static void createBTree1(Node node) {
		Node n2 = new Node(2);
		Node n3 = new Node(3);
		Node n4 = new Node(4);
		Node n5 = new Node(5);
	...
	static Node createBinaryTree(Node node, int height) {
		if (height == 0) {
			return node;
		}
		height--;
		...
		createBinaryTree(node.left, height);
		createBinaryTree(node.right, height);
		return node;
	}
}
